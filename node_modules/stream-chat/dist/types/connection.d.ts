 ...
 * - sometimes your WS connection will seem to work while the userEntity is in fact offline
...
export declare class StableWSConnection<ChannelType extends UnknownType = UnknownType, CommandType extends string = LiteralStringForUnion, UserType extends UnknownType = UnknownType> {
    ...
    userEntity: Constructor<ChannelType, CommandType, UserType>['userEntity'];
    ...
    ws?: WebSocket;
    wsID: number;
    constructor({ apiKey, authType, clientID, eventCallback, logger, messageCallback, recoverCallback, tokenManager, userEntity, userAgent, userID, wsBaseURL, }: Constructor<ChannelType, CommandType, UserType>);
    /**
     ...
     * - `refreshToken` {boolean}	reload/refresh userEntity token be refreshed before attempting reconnection.
     */
    _reconnect(options?: {
        interval?: number;
