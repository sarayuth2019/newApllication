...
> {
  ...
  read: Immutable.ImmutableObject<{
    [key: string]: Immutable.Immutable<{ last_read: Date; userEntity: UserResponse<UserType> }>;
  }>;
  ...
  _addReactionToMessage(
    ...
    if (this._channel.getClient().userID === reaction.userEntity?.id) {
  ...
  _removeReactionFromMessage(
    ...
    const filterReaction = (old: ReactionResponse<ReactionType, UserType>[]) =>
      old.filter(
        (item) => item.type !== reaction.type || item.userEntity?.id !== reaction.userEntity?.id,
      );
