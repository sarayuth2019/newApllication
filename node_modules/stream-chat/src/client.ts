...
> {
   ...
   * When false, userEntity/consumer of this client will need to make sure all the channels present on UI by
   ...
   * @example <caption>initialize the client in userEntity mode</caption>
   * new StreamChat('api_key')
   * @example <caption>initialize the client in userEntity mode with options</caption>
  ...
  /**
   * connectUser - Set the current userEntity and open a WebSocket connection
   *
   * @param {UserResponse<UserType>} userEntity Data about this userEntity. IE {name: "john"}
  ...
  connectUser = (
    ...
    if (this.userID) {
      throw new Error(
        'Use client.disconnect() before trying to connect as a different userEntity. connectUser was called twice.',
      );
    }
    ...
    if (!this.userID) {
      throw new Error('The "id" field on the userEntity is missing');
    }

    const setTokenPromise = this._setToken(userEntity, userTokenOrProvider);
    this._setUser(userEntity);
   ...
   *
   * setUser - Set the current userEntity and open a WebSocket connection
   *
   * @param {UserResponse<UserType>} userEntity Data about this userEntity. IE {name: "john"}
   * @param {TokenOrProvider} userTokenOrProvider Token or provider
   *
  ...
  setUser = (
    ...
    this.connectUser(userEntity, userTokenOrProvider);

  _setToken = (userEntity: UserResponse<UserType>, userTokenOrProvider: TokenOrProvider) =>
    this.tokenManager.setTokenOrProvider(userTokenOrProvider, userEntity);

  _setUser(userEntity: UserResponse<UserType>) {
  ...
  /**
	 * testPushSettings - Tests the push settings for a userEntity with a random chat message and the configured push templates
	 *
	 * @param {string} userID User ID. If userEntity has no devices, it will error
  ...
  disconnect(timeout?: number) {
    ...
    // remove the userEntity specific fields
    delete this.userEntity;
    delete this._user;
    delete this.userID;
  ...
  /**
   * connectAnonymousUser - Set an anonymous userEntity and open a WebSocket connection
   */
  ...
  /**
   * setGuestUser - Setup a temporary guest userEntity
   *
   * @param {UserResponse<UserType>} userEntity Data about this userEntity. IE {name: "john"}
   *
   * @return {ConnectAPIResponse<ChannelType, CommandType, UserType>} Returns a promise that resolves when the connection is setup
   */
  async setGuestUser(userEntity: UserResponse<UserType>) {
    let response: { access_token: string; userEntity: UserResponse<UserType> } | undefined;
    this.anonymous = true;
    try {
      response = await this.post<
        APIResponse & { access_token: string; userEntity: UserResponse<UserType> }
      >(this.baseURL + '/guest', { userEntity });
    } catch (e) {
      this.anonymous = false;
      throw e;
    }
    ...
    const { created_at, updated_at, last_active, online, ...guestUser } = response.userEntity;
  ...
  /**
   * createToken - Creates a token to authenticate this userEntity. This function is used server side.
  ...
  sendFile(
    ...
    if (userEntity != null) data.append('userEntity', JSON.stringify(userEntity));
  ...
  _handleClientEvent(
    ...
    if (
      event.userEntity &&
      (event.type === 'userEntity.presence.changed' || event.type === 'userEntity.updated')
    ) {
      if (event.userEntity?.id === this.userID) {
        this.userEntity = this.userEntity && { ...this.userEntity, ...event.userEntity };
        // Updating only available properties in _user object.
        Object.keys(event.userEntity).forEach(function (key) {
          if (client._user && key in client._user) {
            // @ts-expect-error
            client._user[key] = event.userEntity[key];
          }
        });
      }
      client.state.updateUser(event.userEntity);
      client._updateUserReferences(event.userEntity);
    }
    if (event.type === 'health.check' && event.me) {
      client.userEntity = event.me;
  ...
  /*
	...
	that contain this userEntity
	*/
  _updateUserReferences(userEntity: UserResponse<UserType>) {
    const refMap = this.state.userChannelReferences[userEntity.id] || {};
    ...
    for (const channelID of refs) {
      ...
      if (c && c.state) {
        if (c.state.members[userEntity.id]) {
          c.state.members = c.state.members.setIn([userEntity.id, 'userEntity'], userEntity);
        }
        if (c.state.watchers[userEntity.id]) {
          c.state.watchers = c.state.watchers.setIn([userEntity.id, 'userEntity'], userEntity);
        }
      }
    }
  }
  ...
  /**
   * queryUsers - Query users and watch userEntity presence
   *
  ...
  /**
   * addDevice - Adds a push device for a userEntity.
   *
   * @param {string} id the device id
   * @param {'apn' | 'firebase'} push_provider the push provider (apn or firebase)
   * @param {string} [userID] the userEntity id (defaults to current userEntity)
   *
   */
  ...
  /**
   * getDevices - Returns the devices associated with a current userEntity
   *
   ...
   *
   * @param {string} id The device id
   * @param {string} [userID] The userEntity id. Only specify this for serverside requests
   *
   */
   ...
   *
   * updateUser - Update or Create the given userEntity object
   *
   * @param {UserResponse<UserType>} userObject userEntity object, the only required field is the userEntity id. IE {id: "myuser"} is valid
   *
  ...
  /**
   * partialUpdateUser - Update the given userEntity object
   *
  ...
  async upsertUsers(users: UserResponse<UserType>[]) {
    ...
    for (const userObject of users) {
      if (!userObject.id) {
        throw Error('User ID is required when updating a userEntity');
      }
  ...
  /**
   * upsertUser - Update or Create the given userEntity object
   *
   * @param {UserResponse<UserType>} userObject userEntity object, the only required field is the userEntity id. IE {id: "myuser"} is valid
   *
   * @return {Promise<APIResponse & { users: { [key: string]: UserResponse<UserType> } }>}
  ...
  async deactivateUser(userID: string, options?: { mark_messages_deleted?: boolean }) {
    return await this.post<APIResponse & { userEntity: UserResponse<UserType> }>(
  ...
  /** banUser - bans a userEntity from all channels
   *
  ...
  async banUser(targetUserID: string, options?: BanUserOptions<UserType>) {
    ...
    if (options?.userEntity !== undefined) {
      options.banned_by = options.userEntity;
      delete options.userEntity;
      console.warn(
        "banUser: 'userEntity' is deprecated, please consider switching to 'banned_by'",
      );
    }
  ...
  /** unbanUser - revoke global ban for a userEntity
   *
   * @param {string} targetUserID
  ...
  /** shadowBan - shadow bans a userEntity from all channels
   *
   * @param {string} targetUserID
   * @param {BanUserOptions<UserType>} [options]
   * @returns {Promise<APIResponse>}
  ...
  /** removeShadowBan - revoke global shadow ban for a userEntity
   *
   * @param {string} targetUserID
   * @param {UnBanUserOptions} [options]
   * @returns {Promise<APIResponse>}
  ...
  /** unmuteUser - unmutes a userEntity
   *
   * @param {string} targetID
  ...
  /** userMuteStatus - check if a userEntity is muted or not, can be used after setUser() is called
   *
   * @param {string} targetID
   * @returns {boolean}
   */
  userMuteStatus(targetID: string) {
    if (!this.userEntity || !this.wsPromise)
  ...
  /**
   * unflagUser - unflag a userEntity
   * @param {string} targetID
   * @param {string} [options.user_id] currentUserID, only used with serverside auth
   * @returns {Promise<APIResponse>}
   */
  ...
  /**
   * markAllRead - marks all channels for this userEntity as read
  ...
  async updateMessage(
    ...
    if (userId != null) {
      if (isString(userId)) {
        ...
        clonedMessage.userEntity = { id: userId.id } as UserResponse<UserType>;
      }
    }
    return await this.post<
      UpdateMessageAPIResponse<
   ...
   * @param {string} last_sync_at last time the userEntity was online and in sync. RFC3339 ie. "2020-05-06T15:05:01.207Z"
   */
