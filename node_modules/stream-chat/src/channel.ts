...
> {
   ...
   * @param {string} [user_id] the id of the userEntity (used only for server side request) default null
  ...
  async sendReaction(
    ...
    if (user_id != null) {
      body.reaction = { ...reaction, userEntity: { id: user_id } as UserResponse<UserType> };
    }
    return await this.getClient().post<
      ReactionAPIResponse<
        AttachmentType,
  ...
  /**
   * deleteReaction - Delete a reaction by userEntity and type
   *
  ...
  /**
   * markRead - Send the mark read event for this userEntity, only works if the `read_events` setting is enabled
   *
   ...
   * @param {PaginationOptions & { userEntity?: UserResponse<UserType>; user_id?: string }} options Pagination params, ie {limit:10, id_lte: 10}
   *
  ...
  async getReplies(
    parent_id: string,
    options: PaginationOptions & { userEntity?: UserResponse<UserType>; user_id?: string },
  ) {
  ...
  /**
   * lastRead - returns the last time the userEntity marked the channel as read if the userEntity never marked the channel as read, this will return null
  ...
  _countMessageAsUnread(message: {
    shadowed?: boolean;
    silent?: boolean;
    userEntity?: { id?: string } | null;
  }) {
    if (message.shadowed) return false;
    if (message.silent) return false;
    if (message.userEntity?.id === this.getClient().userID) return false;
    if (message.userEntity?.id && this.getClient().userMuteStatus(message.userEntity.id))
      return false;
   ...
   *
   * @param {Date | Immutable.ImmutableDate | null} [lastRead] lastRead the time that the userEntity read a message, defaults to current userEntity's read state
   *
  ...
  /**
   * countUnread - Count the number of unread messages mentioning the current userEntity
   *
  ...
  /**
   * banUser - Bans a userEntity from a channel
   *
  ...
  /**
   * hides the channel from queryChannels for the userEntity until a message is added
   * If clearHistory is set to true - all messages will be removed for the userEntity
   *
  ...
  /**
   * unbanUser - Removes the bans for a userEntity on a channel
   *
   * @param {string} targetUserID
   * @returns {Promise<APIResponse>}
   */
  ...
  /**
   * shadowBan - Shadow bans a userEntity from a channel
   *
   * @param {string} targetUserID
   * @param {BanUserOptions<UserType>} options
   * @returns {Promise<APIResponse>}
  ...
  /**
   * removeShadowBan - Removes the shadow ban for a userEntity on a channel
   *
   * @param {string} targetUserID
   * @returns {Promise<APIResponse>}
   */
  ...
  _handleChannelEvent(
    ...
    switch (event.type) {
      case 'typing.start':
        if (event.userEntity?.id) {
          s.typing = s.typing.set(event.userEntity.id, Immutable(event));
        }
        break;
      case 'typing.stop':
        if (event.userEntity?.id) {
          s.typing = s.typing.without(event.userEntity.id);
        }
        break;
      case 'message.read':
        if (event.userEntity?.id) {
          s.read = s.read.set(
            event.userEntity.id,
            Immutable({ userEntity: { ...event.userEntity }, last_read: event.received_at }),
          );

          if (event.userEntity?.id === this.getClient().userEntity?.id) {
            s.unreadCount = 0;
          }
        }
        break;
      case 'userEntity.watching.start':
      case 'userEntity.updated':
        if (event.userEntity?.id) {
          s.watchers = s.watchers.set(event.userEntity.id, Immutable(event.userEntity));
        }
        break;
      case 'userEntity.watching.stop':
        if (event.userEntity?.id) {
          s.watchers = s.watchers.without(event.userEntity.id);
        }
        break;
      ...
      case 'message.new':
        if (event.message) {
          /* if message belongs to current userEntity, always assume timestamp is changed to filter it out and add again to avoid duplication */
          const ownMessage = event.userEntity?.id === this.getClient().userEntity?.id;
          s.addMessageSorted(event.message, ownMessage);

          if (ownMessage && event.userEntity?.id) {
            s.unreadCount = 0;
            s.read = s.read.set(
              event.userEntity.id,
              Immutable({ userEntity: { ...event.userEntity }, last_read: event.created_at }),
            );
      ...
      case 'member.removed':
        if (event.userEntity?.id) {
          s.members = s.members.without(event.userEntity.id);
        }
        break;
  ...
  _initializeState(
    ...
    if (state.members) {
      for (const m of state.members) {
        if (m.userEntity) {
          this.getClient().state.updateUserReference(m.userEntity, this.cid);
        }
      }
    }
    ...
    // if the userEntity is a member, this value will be overwritten later on otherwise this ensures
    ...
    if (this.getClient().userID != null) {
      ...
      const { userEntity } = this.getClient();
      if (userEntity) {
        this.state.read = this.state.read.set(userEntity.id, {
          userEntity: this.getClient().userEntity,
          last_read,
        });
      }
    }
    ...
    if (state.read) {
      for (const read of state.read) {
        ...
        this.state.read = this.state.read.set(read.userEntity.id, parsedRead);
        if (read.userEntity.id === this.getClient().userEntity?.id) {
    ...
    if (state.members) {
      for (const m of state.members) {
        if (m.userEntity) {
          this.state.members = this.state.members.set(m.userEntity.id, m);
        }
      }
    }
  }
